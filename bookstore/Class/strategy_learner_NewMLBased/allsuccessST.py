...
class StrategyLearner(object):
    ...
    def get_features_data(self, sd, ed, syms, lookback):
        ...
        bollinger10 = get_bollinger(price, syms, lookback=bollinger_win)[sd:]
    ...
    def testPolicy(self, symbol = "IBM", \
        ...
        buy_val = 0.08 # 2 pass 0.15
        sell_val = - 0.2 # 2 pass -0.3
        for i in newData.index[1:]:
            if newData.get_value(i, 'DailyReturns') >= buy_val  and newData.get_value(i - 1, 'Holdings') == 0:
                newData.set_value(i, "Trades", 1000)
                newData.set_value(i, "BSC", 1)
                newData.set_value(i, 'Holdings', newData.get_value(i - 1, 'Holdings') + newData.get_value(i, 'Trades'))
            elif newData.get_value(i, 'DailyReturns') >= buy_val and newData.get_value(i - 1, 'Holdings') == 1000:
                newData.set_value(i, "Trades", 0)
                newData.set_value(i, "BSC", 1)
                newData.set_value(i, 'Holdings', newData.get_value(i - 1, 'Holdings') + newData.get_value(i, 'Trades'))
            elif newData.get_value(i, 'DailyReturns') < sell_val and newData.get_value(i - 1, 'Holdings') == 1000:
            ...
            elif newData.get_value(i, 'DailyReturns') < sell_val and newData.get_value(i - 1, 'Holdings') == 0:
                newData.set_value(i, "Trades", -1000)
                newData.set_value(i, 'Holdings', newData.get_value(i - 1, 'Holdings') + newData.get_value(i, 'Trades'))
                newData.set_value(i, "BSC", -1)
            else:
